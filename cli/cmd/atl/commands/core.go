...
// autoDrainNotice is the per-turn signal telling the agent to drain one capture
// channel in the background — empty when that channel is empty (no false-fire).
// Both emitters (the UserPromptSubmit tick and session-start) call it for every
// active channel, and the rule the channel names is what acts on it. Core emits
// the signal; it never learns which team ships the rule.
func autoDrainNotice(ch manifest.Channel, pending int) string {
	if pending <= 0 {
		return ""
	}
	return fmt.Sprintf("atl: %d %s(s) pending — auto-drain them now in a background subagent (per the %s rule)",
		pending, ch.Name, ch.Rule)
}
