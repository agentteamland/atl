...
// A team's declared capture channels are recorded into the install manifest, so
// the platform can emit each channel's signal without knowing which team owns
// it. Both write paths must record it — the fresh install and the re-install —
// or a re-install would silently drop the declaration and that channel would go
// dark: its markers refused, its signals never fired, with nothing to say why.
func TestInstallAtRecordsDeclaredChannels(t *testing.T) {
	src := makeSrc(t)
	// Re-declare with a channel (makeSrc's team declares none).
	if err := os.WriteFile(filepath.Join(src, "team.json"),
		[]byte(`{"name":"demo","version":"2.0.0","scope":"project","capabilities":{"profile":{
		  "store":"~/.atl/profiles",
		  "channel":{"name":"profile-fact","drain":"/profile-drain","rule":"profile-capture","describes":"durable entity facts"}}}}`), 0o644); err != nil {
		t.Fatal(err)
	}
	tm, err := teampkg.ReadManifest(src)
	if err != nil {
	...
	for _, pass := range []string{"fresh install", "re-install"} {
		...
		if len(m.Channels) != 1 || m.Channels[0] != want {
			t.Fatalf("%s: manifest channels = %+v, want [%+v]", pass, m.Channels, want)
		}
		if m.SchemaVersion != manifest.SchemaVersion {
			t.Fatalf("%s: schemaVersion = %d, want %d", pass, m.SchemaVersion, manifest.SchemaVersion)
		}
	}
}
