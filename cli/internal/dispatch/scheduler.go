...
- The engine handed you only this work-item's id; fetch everything else from the tracker at runtime: claim the item (transition it to the runtime-resolved in-progress state plus a claim comment), then read the work-item, its **[Technical Analysis]** sentinel comment (read from the work-item, or — for a decomposed child unit that has none of its own — from its nearest ancestor bearing one via the parent-containment link, the way the adapter directs), and the tech-lead's **[Canonical Brief]** sentinel comment (both matched by their exact first-line sentinel, never "the newest comment"); resolve your area:<name> tag and load ONLY that area's pack under %[5]s/<area>/; read the brief-named Architecture/ and Conventions/ durable-knowledge pages.
...
- Re-derive intent FRESH from the tracker — never inherit the developer's: read the work-item (its acceptance criteria = the spec), its **[Technical Analysis]** sentinel comment (read from the work-item, or — for a decomposed child unit that has none of its own — from its nearest ancestor bearing one via the parent-containment link, the way the adapter directs), and the tech-lead's **[Canonical Brief]** sentinel comment (both matched by their exact first-line sentinel, never "the newest comment"); resolve the area:<name> tag.
