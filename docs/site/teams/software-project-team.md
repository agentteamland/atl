...
- **Self-updating learning loop alignment** (1.2.0+) — Every agent's `agent.md` Knowledge Base section is auto-rebuilt by [/drain](/skills/drain) from each `children/{topic}.md` file's `knowledge-base-summary` frontmatter. 169 children files migrated to the new contract via the platform-wide Phase 2.C migration script (lossless, summaries lifted verbatim from existing hand-curated KB sections). Every skill ships an empty `learnings/` subdirectory with `## Accumulated Learnings` section ready for `/drain` to populate.
