...
**`release`** — never on a conversational approval, but on a commit-bound approval record read back from
the backend (the gate below; v1 binds it on **GitHub**, and **holds** the promotion on Azure until the
missing read is bound there). Review is **delivery-native**: the tech-lead runs the
...
compiles the report, opens (or finds) the `dev` → `release` **promotion PR**, and then reads a durable
**approval record** back from the backend. Opening that PR is no longer the promotion — **merging it is**.
In v1 the gate is bound on the **GitHub** backend only — see **GitHub-only in v1** at the end of this
section for what is still missing on Azure.
...
The ceremony then compares the commit named in the record with the promotion PR's **current head** and
merges only on an exact match — SHA-pinned (`gh pr merge --merge --match-head-commit <approved-commit>`),
so the provider itself refuses the merge if the head moved in between. The outcome is recorded on the
sprint's review page under `## Promotion decision`: approved commit, approver, timestamp, PR link. Every
other outcome is a **HOLD** — nothing merges, no work-item changes state, and the ceremony prints exactly
what you have to set:

| What the ceremony reads back | What it does |
|---|---|
| No `**[Promotion Approval]**` record on the PR | Holds, and prints the PR link + the head commit to approve. |
| A record with no 40-hex id under `## Approved Commit` | Holds, and asks for a re-post naming the current head. |
| A record naming a commit that is not the PR's head | Holds — the approved state is not the state that would ship. |
| The record could not be read | Holds. Unverified is not approved. |
...
**When `dev` advances after an approval, the approval expires with it.** The ceremony reports the approved
commit, the current head, and how far `dev` has moved, and it does **not** carry the approval forward. To
promote the current state, re-read the refreshed report and add a new record for the new head; to promote
exactly what you approved, reset `dev` back to that commit first. The stale record stays in place as audit
history — the channel is append-and-supersede, and the next record supersedes it by naming a newer commit.
...
That is a **HOLD, not a fallback** — an unresolvable head is a read failure, the last row of the table
above, so on Azure `/sprint-review` compiles the report, opens (or finds) the promotion PR, and then holds:
it does **not** revert to promoting on a conversational approval. Until the read is bound, an Azure project
on this version promotes by completing that promotion PR itself, in Azure DevOps. Resolving the field
against a live server and binding it is the named next step.
