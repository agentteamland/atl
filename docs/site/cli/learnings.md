...
The queue is one embedded [bbolt](https://github.com/etcd-io/bbolt) file at `~/.atl/queue.db` — no server, no daemon. Every project's queue lives in that one file, isolated into per-project buckets keyed by the working directory. All three subcommands operate on the **current project** (the directory you run them in).
