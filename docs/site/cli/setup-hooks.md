...
6. **Retrieval index refresh (background)** — when this project's knowledge corpus (wiki + journal) changed since the last build, spawns a detached `atl retrieve index` so the per-prompt retrieval hook has a fresh index. Throttled, skipped inside git worktrees, and disabled with `ATL_NO_RETRIEVE_INDEX`.
...
Alongside the tick, a second `UserPromptSubmit` entry runs `atl retrieve`: it ranks this project's knowledge pages (wiki + journal) against each prompt — BM25 fused with a local semantic embedder — and surfaces the top matches as context, so Claude consults the most relevant pages before answering. It's fail-open: any error prints nothing and never blocks the prompt. See the [knowledge-system guide](/guide/knowledge-system).
