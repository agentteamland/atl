...
5. **Capture watchdog.** On the newest scanned transcript (the live session), measures the **dry stretch** — logical assistant turns and user-authored chars since the last marker-bearing turn, computed fresh from the file each pass (no counters to drift). If **both** thresholds are exceeded (≥2 assistant turns AND ≥1000 chars of user-authored input), prints a one-line nudge to review the recent turns for missed learnings/profile-facts and spawn a background [`/drain`](/skills/drain) whose mining step sweeps the stretch (valid even with an empty queue). Fires **once per dry stretch** — a latch keyed by (session file, marker ordinal) re-arms whenever a new marker appears or a new session starts, so it never nags. This closes the pipeline's one non-deterministic link: a marker the agent forgot to write is no longer silent. `ATL_NO_CAPTURE_WATCHDOG=1` opts out.
