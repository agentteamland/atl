...
4. **Write assets.** The team's installable subtrees — `agents/`, `skills/`, `rules/`, `knowledge/`, `scripts/`, and `packs/` — are copied straight into the scope's Claude Code directory — `~/.claude` for a global install, `<project>/.claude` for a project install. Nothing else from the repo (`team.json`, `README`, `LICENSE`) is copied.
...
- **`team ships no installable assets`** — the resolved team has none of the installable subtrees (`agents/`, `skills/`, `rules/`, `knowledge/`, `scripts/`, `packs/`).
