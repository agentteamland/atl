...
4. **Write assets.** The team's installable subtrees — `agents/`, `skills/`, `rules/`, `knowledge/`, `backends/`, `scripts/`, and `packs/` — are copied straight into the scope's Claude Code directory — `~/.claude` for a global install, `<project>/.claude` for a project install. Nothing else from the repo (`team.json`, `README`, `LICENSE`) is copied.
...
6. **Bind automation hooks.** The automation hooks (`SessionStart → atl session-start`, `UserPromptSubmit → atl tick`, `UserPromptSubmit → atl retrieve`, and `PreToolUse (Bash|Edit|Write) → atl guard`) are installed into Claude Code as a mandatory part of install — automation is on by default, not opt-in. A hook-binding failure is surfaced as a warning and does not fail the install.
...
- **`team ships no installable assets`** — the resolved team has none of the installable subtrees (`agents/`, `skills/`, `rules/`, `knowledge/`, `backends/`, `scripts/`, `packs/`).
