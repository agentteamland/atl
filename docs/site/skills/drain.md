...
- When `atl` fires the **capture-watchdog signal** (`atl: capture-watchdog (learning) — no learning markers for N assistant turn(s) …`): a substantive stretch passed with no learning markers, so the queue may be **empty** — the mining step is the run's purpose; it sweeps the flagged stretch and recovers what should have been marked. The watchdog measures each capture channel separately, so a `(profile-fact)` signal is [`/profile-drain`](/teams/profile-team)'s trigger rather than this skill's; both can fire on the same turn.
