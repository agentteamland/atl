...
Skills can be **global** (shipped with `atl` itself) or **team-scoped** (shipped by a specific team and only visible after the team is installed). Team-scoped skills — like delivery-team's `/kickoff` and `/refine`, or personal-advisory-team's `/advisor` — appear only after their team is installed, because the work they do depends on that team's agents and stack. `/drain`, `/create-pr`, `/create-code-diagram`, `/brainstorm`, `/rule`, `/rule-wizard`, `/docs-audit`, `/publish`, `/skill-stocktake`, and `/rules-distill` are global because they apply universally.
