...
**When the board has drifted out of a sprint, session-start says so.** On the GitHub backend under the `flow` sprint mode, ATL reads the board out of band (once a day, in a detached process, so nothing slows the session down) and caches the two facts it needs: how much work is open, and which `sprint:<n>` label is the highest. Combine that with which sprint review pages exist in your repo (`docs/sprints/sprint-<n>-review.md`) and you get the one condition worth reporting — **the board has open work, and no sprint is currently open.** Session-start then prints a line naming `/sprint-plan`.

It is a **report, not a task the agent should run**. Which work enters a sprint is the product owner's decision, so the line asks you to raise it with them rather than telling an agent to go and open one. The signal is silent while a sprint is active, silent when nothing is open, and silent in any project without a `.delivery/config.json`. It never blocks or fails the hook: no board access, no `gh`, an expired token, or a slow network all resolve to saying nothing. Set `ATL_NO_SPRINT_SIGNAL` to switch off both the report and the board read behind it.

Two configurations it deliberately does not cover, and the code says so where it refuses. **Azure** reaches its board through an MCP surface, which is an LLM-side tool with no equivalent of `gh` for a Go process to call — answering the question there needs a REST client that does not exist yet. And under the **`scrum`** mode the sprint carrier is the Projects v2 Iteration field rather than a `sprint:<n>` label, so the label predicate would find nothing and wrongly report "no active sprint" on every scrum board. In both cases the signal stays quiet instead of guessing.
