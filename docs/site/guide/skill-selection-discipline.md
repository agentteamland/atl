...
If you know which slash command to use, invoking it directly is faster and more reliable than waiting for the assistant to remember. Typing `/drain` runs immediately; "drain my learnings" requires the assistant to map intent to skill name.
...
- **Rule source:** [`core/rules/skill-selection-discipline.md`](https://github.com/agentteamland/atl/blob/main/core/rules/skill-selection-discipline.md) — the assistant-side rule that this page is the user-side counterpart of.
