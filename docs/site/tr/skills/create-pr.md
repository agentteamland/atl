...
Çalışma ağacı değişikliklerini al (commit edilmemiş ya da yakın zamanda varsayılan dala commit edilmiş), farktan uygun bir dal adı + commit mesajı + PR başlığı türet, [`/drain`](/tr/skills/drain) komutunu çalıştır ki bekleyen öğrenmeler aynı PR'ın içinde yolculuk etsin, bir AI inceleme zinciri çalıştır (genel temel + takımın bildirdiği uzmanlar), commit + push yap, bir PR aç. İsteğe bağlı olarak GitHub auto-merge düzeneğini, sınırlı bir yoklama + kendiliğinden düzeltme döngüsüyle etkinleştir. İş bitiminde kullanıcıyı daima hedef dala döndür.
