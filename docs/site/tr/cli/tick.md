...
5. **Yakalama bekçisi (capture watchdog).** Taranan en yeni transkript (canlı oturum) üzerinde **kuru seriyi** ölçer — son işaretçi taşıyan turdan bu yana geçen mantıksal assistant turları ve kullanıcının yazdığı karakterler; her geçişte dosyadan taze hesaplanır (sapacak sayaç yok). **Her iki** eşik de aşıldığında (≥2 assistant turu VE kullanıcının yazdığı ≥1000 karakter), son turları kaçırılmış learning/profile-fact'ler için gözden geçirme ve mining adımı seriyi tarayacak bir arka plan [`/drain`](/tr/skills/drain)'i başlatma çağrısı yapan tek satırlık bir dürtme yazdırır (kuyruk boşken bile geçerli). **Kuru seri başına bir kez** ateşlenir — (oturum dosyası, işaretçi sırası) anahtarlı mandal, yeni bir işaretçi ya da yeni bir oturumla kendiliğinden yeniden kurulur; asla dırdır etmez. Bu, boru hattının tek deterministik-olmayan halkasını kapatır: ajanın yazmayı unuttuğu bir işaretçi artık sessiz kalmaz. `ATL_NO_CAPTURE_WATCHDOG=1` devre dışı bırakır.
