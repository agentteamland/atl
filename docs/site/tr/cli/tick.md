...
2. **Otomatik-drain sinyali (her çağrıda).** **Etkin her yakalama kanalı** için — çekirdeğin kendi `learning` kanalı ve kurulu bir takımın bildirdiği her kanal — o kanalda bekleyen öğe varsa tek satırlık bir otomatik-drain sinyali yazdırır (`atl: N learning(s) pending — auto-drain them now in a background subagent (per the learning-capture rule)`); böylece ajan o kanalın drain'ini arka planda bir alt-ajan olarak başlatır. Her cümle kanalın kendi bildiriminden kurulur, dolayısıyla platform hiçbir takımı adlandırmaz: hangi kanalın beklediğini ve hangi kuralın devreye gireceğini söyler; o kural da kanalın sahibi olan takımla birlikte gelir ([yakalama kanalı bildirmek](/tr/authoring/team-json#declaring-a-capture-channel)). Tek bir ucuz sayaç okumasıdır ve kısıtlama kapısından **önce** çalışır; bu yüzden kuyruk boş olmadığı her mesajda tetiklenir — kısıtlama onu asla bastırmaz.
...
5. **Yakalama bekçisi (capture watchdog).** Taranan en yeni transkript (canlı oturum) üzerinde **kuru seriyi** ölçer — son işaretçi taşıyan turdan bu yana geçen mantıksal assistant turları ve kullanıcının yazdığı karakterler; her geçişte dosyadan taze hesaplanır (sapacak sayaç yok). Seri **her yakalama kanalı için ayrı** ölçülür; böylece bir `learning` işaretçisi bir takımın kanalında kaçırılmış bir işaretçiyi maskeleyemez: etkin her kanalın kendi serisi, kendi mandalı ve bildiriminden gelen drain becerisi ile kuralı adlandıran kendi dürtmesi olur (çekirdeğin `learning` kanalı [`/drain`](/tr/skills/drain)'i adlandırır). İki kanal aynı turda ateşleyebilir — farklı subagent'lardır. Bir kanal için **her iki** eşik de aşıldığında (≥2 assistant turu VE kullanıcının yazdığı ≥1000 karakter), son turları işaretlenmemiş kalanlar için gözden geçirme ve mining adımı seriyi tarayacak o arka plan drain'ini başlatma çağrısı yapan tek satırlık bir dürtme yazdırır (kuyruk boşken bile geçerli). **Kanal başına, kuru seri başına bir kez** ateşlenir — (oturum dosyası, kanal, işaretçi sırası) anahtarlı mandal, yeni bir işaretçi ya da yeni bir oturumla kendiliğinden yeniden kurulur; asla dırdır etmez. Bu, boru hattının tek deterministik-olmayan halkasını kapatır: ajanın yazmayı unuttuğu bir işaretçi artık sessiz kalmaz. `ATL_NO_CAPTURE_WATCHDOG=1` devre dışı bırakır.

   **"Kullanıcının yazdığı", "role: user"dan dardır.** Kullanıcı-rollü transkript kayıtlarının çoğu makine tarafından enjekte edilir ve onları saymak karakter eşiğini formaliteye çevirir. Dört taşıyıcı dışlanır: `isMeta` kayıtları (bir becerinin `SKILL.md` dökümü), etiketle açılan diziler (görev bildirimleri), uzun bir oturum sıkıştırıldığında harness'ın yazdığı **sıkıştırma özeti** (gözlenen 13.995–17.966 karakter — tek başına eşiği geçiyor) ve `[Request interrupted by user]` iptal bildirimi. Gerçek bir korpusta bunlar, kapının saydığı her şeyin **%45'iydi**.
6. **Doctor öz-denetimi.** [`atl doctor`](/tr/cli/doctor) ve [`atl session-start`](/tr/cli/setup-hooks) ile aynı kuyruk-sağlığı + varlık-bütünlüğü denetimlerini çalıştırır; yalnızca OK olmayan (ya da kendini onaran) satırları yazdırır. Her şey sağlıklıyken sessizdir.
7. **Kazanımları yükselt (1→2. halka).** Bu projenin biriken kazanımlarını kullanıcı-global katmana çıkarır. Eklemeli ve çakışma-arşivlidir; bu yüzden elle bir [`atl promote`](/tr/cli/promote) beklemek yerine tick'e eşlik etmesi güvenlidir. Çıkarılacak bir şey olmadığında sessizdir.
