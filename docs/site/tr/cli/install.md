...
4. **Varlıkları yaz.** Takımın kurulabilir ağaçları — `agents/`, `skills/`, `rules/`, `knowledge/`, `scripts/` ve `packs/` — doğrudan kapsamın Claude Code dizinine kopyalanır — global kurulum için `~/.claude`, proje kurulumu için `<project>/.claude`. Depodan başka hiçbir şey (`team.json`, `README`, `LICENSE`) kopyalanmaz.
...
- **`team ships no installable assets`** — çözümlenen takımda kurulabilir ağaçların hiçbiri yok (`agents/`, `skills/`, `rules/`, `knowledge/`, `scripts/`, `packs/`).
