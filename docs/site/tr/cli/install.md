...
4. **Varlıkları yaz.** Takımın kurulabilir ağaçları — `agents/`, `skills/`, `rules/`, `knowledge/`, `backends/`, `scripts/` ve `packs/` — doğrudan kapsamın Claude Code dizinine kopyalanır — global kurulum için `~/.claude`, proje kurulumu için `<project>/.claude`. Depodan başka hiçbir şey (`team.json`, `README`, `LICENSE`) kopyalanmaz.
...
Bir takım `team.json`'ında başka takımları `dependencies` (takım adı → semver aralığı eşlemesi) olarak bildirebilir. `atl install` bunları **transitif** olarak çözüp kurar — bir takımı kurmak, ihtiyaç duyduğu takımları, onların ihtiyaç duyduklarını, ve böyle devam eder, hepsini çeker. Yani `profile-team`'e bağımlı bir danışman takımını kurmak `profile-team`'i otomatik çeker.

- **Bir bağımlılık kendi bildirdiği kapsamda kurulur**, tüketicinin değil. Global bir bağımlılık, tüketiciyi `--project` ile kursan bile global kalır.
- `core` (platform çekirdeği) her zaman mevcuttur — binary'de gelir, atlanır.
- Elmas ve döngüler güvenlidir: her takım bir kez kurulur.
- Index'in çözemediği bir bağımlılık, sert hata değil, bir uyarıyla atlanır.

Bağımlılık alanı için [`team.json`](/tr/authoring/team-json) sayfasına bak.
...
- **`team ships no installable assets`** — çözümlenen takımda kurulabilir ağaçların hiçbiri yok (`agents/`, `skills/`, `rules/`, `knowledge/`, `backends/`, `scripts/`, `packs/`).
