...
Beceriler **global** (`atl`'nin kendisiyle birlikte gelen) ya da **takım kapsamlı** (belirli bir takımca getirilen ve yalnızca o takım kurulduktan sonra görünen) olabilir. Takım kapsamlı beceriler — örneğin delivery-team'in `/kickoff` ve `/refine` becerileri ya da personal-advisory-team'in `/advisor`'ı — yalnızca takımları kurulduktan sonra görünür, çünkü yaptıkları iş o takımın ajanlarına ve yığınına bağlıdır. `/drain`, `/create-pr`, `/create-code-diagram`, `/brainstorm`, `/rule`, `/rule-wizard`, `/docs-audit`, `/publish`, `/skill-stocktake` ve `/rules-distill` ise globaldir çünkü her yere uygulanır.
