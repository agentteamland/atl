...
| 4 | **Typed metadata / tags** | Free-form, queryable, zero-setup labels carrying three machine-contracts: `atl-key:<hash>` idempotency + `atl-run:<…>` provenance, `area:<name>` pack-binding, `blocked`. | `System.Tags` | issue **labels** (queryable via issue advanced search) |
