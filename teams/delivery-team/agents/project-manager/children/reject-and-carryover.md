...
1. The rejected item's **`IterationPath` is cleared** (removed from the sprint) via
   `wit_update_work_item` — an idempotent field update, the same field-not-membership mechanism as
   assignment ([iteration-management.md](iteration-management.md)). With no IterationPath, it is
   back on the backlog.
2. Its state is set back to a not-Done, ready-to-rework category — resolve the concrete state at
   runtime (`wit_get_work_item_type`, adapter §6); never write a literal (the rework state might be
   `New`, `Active`, `Reopened`, or a custom value depending on the process template).
3. The **rejection reason is recorded** as a work-item comment (`wit_add_work_item_comment`), so
   the next developer who picks it up knows *why* it came back — the PO's acceptance gap is the
   context that prevents re-delivering the same miss.
4. At the **next `/sprint-plan`**, the item is simply a backlog candidate again: my blueprint reads
   it with everything else, places it in the DAG, and admits it against capacity by StackRank —
...
1. **Clear or leave the IterationPath** — for a clean re-plan, I clear the incomplete item's
   IterationPath at sprint close so the next `/sprint-plan` picks it up as a fresh backlog
...
review page); the item travels back to the backlog (IterationPath cleared); the next plan treats it
...
- `Y` → IterationPath cleared, state set to the resolved rework category, rejection reason
  commented; appears under `## Carryover` (reason: review/acceptance); re-enters the backlog.
- `Z` → IterationPath cleared; appears under `## Carryover` (reason: blocked on `<predecessor>`);
