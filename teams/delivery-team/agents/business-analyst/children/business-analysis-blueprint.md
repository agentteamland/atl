---
knowledge-base-summary: "My primary production unit: authoring an Epic/Feature's business analysis into the spec field under the five fixed H2s (Problem / Business Value / Scope / Acceptance Criteria / Out of Scope) per concept #2. Covers the Epic→Feature artifact hierarchy, the exact write/read-back contract, an idempotent upsert discipline, a completion checklist, and a generic worked example."
---

# Business Analysis Blueprint

This is my primary production unit — the artifact I create over and over: the **business
analysis of an Epic or Feature**, written into the work-item's **spec field** under a
fixed set of headings. Every consumer downstream (the `technical-analyst`, the `tech-lead`,
the `project-manager`, the `tester`, and the human PO) reads that spec field **by location**,
...
- **Epic** — a large business outcome, months of value, spanning several Features. My
  spec field frames the outcome and the value hypothesis at the coarsest level.
- **Feature** — a coherent shippable slice of that Epic, the level a sprint plans around. My
  spec field here is sharper: concrete scope and testable acceptance criteria.
...
spec field that starts prescribing tasks has leaked out of my lane.

**Resolve the concrete type name at runtime — never hardcode it.** "Epic" and "Feature" are the
abstract rungs; the live project may spell them differently under a different backend or process
template. Resolve the real type name at runtime before creating
(concept #7); the descriptor's `workItemTypeMap` is null-seeded on
purpose for exactly this reason.

## The content-placement contract (concept #2) — the five fixed H2s

The business analysis goes into the **spec field** as Markdown under these **exact** headings,
in this order (concept #2). This is the durable, always-loaded
...
**Why these five, in this order, always the same:** the spec field is business-owned and
...
spec field) precisely so the spec field stays mine and stays business-shaped — I never touch
that comment, and it never touches my spec field. `## Out of Scope` is not optional politeness:
it is the negative space that the `tech-lead`'s decomposition and the `tester`'s coverage both
lean on.

## The write / read-back mechanics

**Write** — set the **spec field** when I create the work-item (on create) or update it (on
refine), per the active backend's adapter op→tool map. I write the whole spec-field block as one field
value; I do not scatter it across fields.

**Read-back** — a consumer reads my analysis (reads the work-item) and parses the spec-field
headings (concept #2). Because the headings are fixed, this is a
deterministic parse, never a heuristic.

**Idempotent upsert (concept #10) — a refine must converge, not duplicate.** When I re-analyze an
existing item at `/refine`, I **update the existing item's spec field**, I do not create a new
item. Creation of *new* backlog items during `/kickoff` goes through the team's idempotency
discipline: every created item carries the deterministic `atl-key:<hash>` tag/label
(`hash = hash(parent-id + plan-ordinal)`) and a check-first idempotency query (concept #10) runs
before any create — found → reuse+update, not-found → create-then-stamp
(concept #10). The plan-ordinal that seeds the key comes from the
...
**Resilience (the resilience policy):** every write wraps exponential backoff + jitter and honours
...
The spec field is **terse by design** — it is the always-loaded summary. The deep,
long-form analysis (personas, scenarios, edge conditions, the full domain reasoning behind a
Feature) lives in the durable-knowledge store's `Analysis/` namespace, which I co-own with the
`technical-analyst` (`analysis-store-craft.md`, and concept #9). The spec field points at value
and testable conditions; the durable-knowledge page carries the reasoning. Never inflate the spec field into
an essay — put the essay in `Analysis/` and keep the spec field scannable.

## Completion checklist (run this before I mark an item analyzed)

- [ ] The concrete work-item type name was resolved at runtime (concept #7), not
      hardcoded — Epic/Feature is the abstract rung, the project's spelling is authoritative.
- [ ] The **spec field** contains **all five** H2s, in order:
...
- [ ] I did **not** apply any `area:<name>` tag — I only *suggest* business scope; the `tech-lead`
      owns area→pack binding at decomposition (concept #4).
- [ ] The write was an idempotent upsert: an existing item was updated in place; a new item ran a
      check-first idempotency query and carries its `atl-key` stamp (concept #10).
- [ ] The deep reasoning (if any) went to the `Analysis/` durable-knowledge page, not into the spec field.
- [ ] If domain terms/entities/rules were established, the `Domain/` durable-knowledge page was updated
