...
- The item's **spec field** — parse the five fixed H2s (read the work-item, concept #2).
- The `Domain/` durable-knowledge namespace I own — has the vocabulary/rules moved since this item was written?
  (read the durable-knowledge store, `domain-modeling.md`.)
...
- The `technical-analyst`'s comment on the item — read it by the `**[Technical Analysis]**`
  **sentinel match**, never "the newest comment" (concept #3), so a later human comment never
...
update** of the existing spec field (update the work-item, concept #10 — I update in place, I
...
- Each resulting item gets its own full five-H2 spec field; the value must **ladder up** to the
  parent (`business-value-framing.md`) — a split child whose value doesn't trace to the parent is
  a mis-cut.
- New items created by a split follow the idempotency contract: the check-first idempotency query + the
  `atl-key:<hash>` stamp keyed on stable `parent + ordinal` (concept #10), so a re-run of `/refine`
...
- On the `Analysis/` page we each revise **our own section only** and cross-link where our layers
  meet (`analysis-store-craft.md`) — read-before-write, idempotent upsert, no write race.
- The **division of labor stays sharp:** I never write the `**[Technical Analysis]**` comment, and
  I never apply an `area:<name>` tag — areas are the `tech-lead`'s at decomposition (concept #4). I
  keep to the business "what & why".

### 5. Keep the Domain durable-knowledge current as understanding deepens
...
- [ ] Re-read the spec field (five H2s), the `Domain/` pages, the `Analysis/` page, and the TA's
...
- [ ] Oversized items split along **business** seams; each split item has a full five-H2
      spec field whose value ladders up to its parent.
- [ ] Every write is an idempotent update/upsert (existing item updated in place; new split items
      carry the check-first idempotency query + `atl-key` stamp, concept #10).
