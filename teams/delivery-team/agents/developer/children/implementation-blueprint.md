...
runtime from the tech-lead's canonical brief, the loaded stack-pack, and the durable-knowledge store;
I never carry them in this file (see [`pack-loading.md`](pack-loading.md),
[`worktree-and-isolation.md`](worktree-and-isolation.md)).

## The 8-step micro-loop

The ordered loop (the backend mechanics of each milestone write live in the active backend's adapter,
`backends/<backend>/adapter.md`; my touchpoints are in
[`backend-touchpoints.md`](backend-touchpoints.md)):

1. **claim** — transition the work-item to the runtime-resolved in-progress state + write a claim
   comment. `phase: claim`. [me]
2. **plan** — assemble my context (task + the tagged area's stack-pack + the tech-lead's canonical
   brief + the brief-named durable-knowledge pages) and derive a plan for *this* unit. `phase: plan`. [me]
3. **implement** — write the change in my worktree, following the pack's conventions atop the
   project's (durable-knowledge) conventions. `phase: implement`. [me]
...
5. **comment** — write a progress comment on the work-item (what I built, what I verified, evidence
   pointers). `phase: comment`. [me]
6. **pr** — open the delivery-native pull request (on the active backend) and link it to the
   work-item. `phase: pr`. [me] **This is where my job ends** — the PR is my handoff to review.
...
8. **[tech-lead merge + engine verify]** — after `green = (all test-gates passed) ∧ (review passed)`,
   ...
   (`MergedToBase`) and gates refill on it (8b) — it never merges (it is zero-backend). **NOT my step.**
...
- **Merge is the tech-lead's; verification is the engine's — durable-state safety.** On green the
  **tech-lead completes the PR (= the merge to `dev`, non-squash)** — the engine is zero-backend
  and cannot complete a PR on the backend. The engine then **verifies the durable git state** (`MergedToBase`
...
- **The ordering `merge-to-dev precedes Done` is strict.** Done is what triggers refill; if Done
  ...
  (concept #7 — the Done state is runtime-resolved; the tech-lead drives the transition after
  completing the PR).
...
- **claim before implement** — claiming (state transition + comment) makes the work-unit's
  ...
  legible (the item shows in-progress, and on re-claim I converge on it — the idempotency concept,
  #10). Silent work that never claimed would look un-started if I died.
- **plan before implement** — I bound my context (pack + brief + durable-knowledge + task) and derive intent
...
- [ ] **claim:** work-item transitioned to the runtime-resolved in-progress state (resolve the
      completion/state model, then set it — concept #7; never a literal), claim comment written
      (add a comment — concept #3). On a re-claim after a crash, converged on the existing item —
      did not duplicate (idempotency — concept #10).
- [ ] **plan:** context assembled from the tagged area's pack + the canonical brief + the
      brief-named durable-knowledge pages + the task; intent derived and traced to the Feature's
      `## Acceptance Criteria`.
- [ ] **implement:** change written in *my worktree only*, following the pack conventions atop the
      project (durable-knowledge) conventions.
- [ ] **self-test:** Level-1 self-test run on every surface the unit touches (code always; web
      ...
      silent-passed** ([`self-test-craft.md`](self-test-craft.md)); evidence attached per the active
      adapter (concept #12).
- [ ] **comment:** progress comment written summarizing the change + verification + evidence
      pointers.
- [ ] **pr:** delivery-native PR opened on the active backend and linked to the work-item
      (concept #11).
...
- [ ] **Did NOT:** review my own PR (step 7 = tech-lead), merge or set Done (step 8a = tech-lead
      ...
      durable-knowledge store ([`learning-routing.md`](learning-routing.md)).
