---
name: sprint-plan
description: /sprint-plan — velocity-driven sprint selection for the delivery-team. Computes the sprint's capacity ceiling from the mean StoryPoints of the last N=3 CLOSED sprints (with a cold-start po-seed decay for the first N), then acting as the project-manager (with the tech-lead for feasibility) selects backlog units by stack-rank up to capacity at a single granularity and assigns each an IterationPath. Reads methodology.capacityModel + prior closed iterations + backlog stack-rank + Architecture/ wiki; writes only the idempotent IterationPath field. A recurring planning ceremony (methodology.cadence.planningCeremonies); velocity is read-only.
---
...
it (stack-rank order, up to that ceiling, at one granularity level), then commits the choice by
setting each selected unit's `IterationPath`. It runs **in-session** and adopts the
...
| It reads | It writes |
|---|---|
| `.delivery/methodology.json` (`capacityModel`, `artifactHierarchy`, `cadence`); the last `velocityWindowN` **CLOSED** iterations; the backlog stack-rank; `Architecture/` wiki pages for feasibility | **only** the `IterationPath` field on the selected units (an idempotent field update). **Velocity is read-only.** |
...
resolve the concrete iteration schedule with `work_list_iterations`
...
completed StoryPoints over the last `velocityWindowN` (=3) CLOSED sprints** (read
`velocityWindowN` from `capacityModel`, never hardcode `3`):

- Enumerate the closed iterations from `work_list_iterations` (the
...
- Optionally read `work_get_team_capacity` as a **secondary corroborating signal** for the
  availability dial (logged days-off), but keep the ceiling **velocity-derived** (Azure capacity is
  an hours model; this team estimates in story points — the two do not convert cleanly).

### 4. Select backlog units by stack-rank up to capacity, at ONE granularity, and assign the iteration
...
feasibility against the `Architecture/` wiki before the assignment is committed.

- **Read the backlog completely** — `wit_list_backlog_work_items` (the stack-rank-ordered backlog)
  and/or a high-`top` `wit_query_by_wiql` filtered to the ready types and the **not-yet-Completed**
  state (resolve the Completed category at runtime via `wit_get_work_item_type`, §6). Apply the
  cap-is-truncation rule (§4).
...
- **Select by stack-rank up to capacity** — take units in ascending `Microsoft.VSTS.Common.StackRank`
  / `BacklogPriority` order (lower value = higher priority; Azure orders the board ascending) until
  the summed `Microsoft.VSTS.Scheduling.StoryPoints` would exceed `capacity`. An item with no
  estimate is a planning gap — surface it, never admit an unestimated unit (its point cost is
  unknown and corrupts the capacity math). Equal/absent StackRank falls back to the stable backlog
  order returned by `wit_list_backlog_work_items`.
- **Feasibility pass (as the `tech-lead`)** — read the relevant `Architecture/` wiki pages
  (`wiki_get_page_content`; `search_wiki` for discovery) and flag any selected unit whose approach
...
- **IterationPath assignment is idempotent by nature** — it is a `System.IterationPath` **field
  update** (`wit_update_work_item`), so re-running sets the same path to the same value: a safe
  no-op. Never model it as a "create membership" that could double (adapter §5).
...
- **This ceremony does not create work-items** in the normal path — it *selects existing* backlog
  ...
  swap-in the tech-lead adds), that item carries the two `System.Tags` the contract requires:
  ...
  never `hash(title)` — and is guarded by a **check-first WIQL** (`wit_query_by_wiql` for that
  `atl-key`: found → reuse+update, not-found → create-then-stamp; a 409/duplicate is caught and
  resolved to the existing item, never surfaced), per adapter §5 and the tech-lead's
