...
[ -f "$HOME/.claude/agents/profile-curator/children/schema-migration.md" ]     && ok "breaking-change migration logic reflected" || bad "schema-migration missing"
grep -q 'Reality gate' "$HOME/.claude/agents/profile-curator/children/marker-drain.md" 2>/dev/null && ok "reality gate (drop doc-example facts) reflected" || bad "reality-gate content missing from marker-drain"
