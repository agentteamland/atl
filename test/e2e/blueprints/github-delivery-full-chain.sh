...
#
# THIS PROMPT AND THE /refine PROMPT BELOW DESCRIBE ONE ARCHITECTURE. Edit them together.
#
# The module-per-helper layout is stated here, in the seeded facts, rather than left to the
# ceremony's judgement — because /kickoff WRITES the Feature's Acceptance Criteria and its
# [Technical Analysis], and /refine then reads them back as binding. On 2026-08-05 this prompt
# named app.js for slice 1 and said nothing about where slices 2 and 3 live; kickoff quite
# reasonably put all three in app.js and wrote an NFR pinning them there ("consistent with the
# library's current zero-dependency footprint"). /refine read that, found it irreconcilable
# with its own instruction to create absdiff.js/iszero.js and to forbid touching app.js, and —
# correctly, per no-fabrication — surfaced the conflict and asked instead of silently picking a
# side. Zero PBIs, and six cascading reds behind them.
#
# The ceremony was right; the blueprint contradicted itself one turn later. Note what let it
# survive earlier runs: kickoff is an LLM turn, so its ACs varied, and the runs that happened
# to leave the file layout unstated passed. A latent defect that only surfaces when the
# fixture-writing turn is MORE specific than usual cannot be found by a green run.
