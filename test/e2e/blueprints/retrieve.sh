...
mkdir -p "$PROJ/.atl/wiki" "$PROJ/docs"
...
MD
# an in-repo docs/ page is NOT part of the corpus — docs/ is never indexed
cat > "$PROJ/docs/architecture.md" <<'MD'
# Architecture store
A repo's docs tree is often a large vendored site, so it stays out of the corpus.
MD
cd "$PROJ"

# build a lexical-only index (deterministic, offline) — wiki only; docs/ is out
out="$(atl retrieve index --lexical 2>&1)"
echo "$out" | grep -q "indexed 2 pages" && ok "index built 2 pages (wiki only)" || bad "index build -- [$out]"

# a second build is idempotent (incremental reuse path stays correct)
out="$(atl retrieve index --lexical 2>&1)"
echo "$out" | grep -q "indexed 2 pages" && ok "re-index is idempotent" || bad "re-index -- [$out]"

# the docs/ page is NOT retrievable — the corpus boundary, asserted rather than
# assumed: docs/ sits right beside .atl/ and is the easiest tree to re-admit by
# accident, and re-admitting it is silent (a bigger index looks like a better one).
out="$(echo '{"prompt":"where does a repo keep its architecture store","cwd":"'"$PROJ"'"}' | atl retrieve 2>&1)"
echo "$out" | grep -q "docs/architecture.md" && bad "docs/ page surfaced -- [$out]" || ok "docs/ stays out of the corpus"
