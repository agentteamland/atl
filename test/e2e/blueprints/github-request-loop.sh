...
CAND=$(gh_try gh issue list --repo "$REPO" --state all --label candidate --json number -q '.[0].number')
[ -n "$CAND" ] && ok "/request captured a candidate issue (#$CAND) with the 'candidate' label" || bad "no candidate issue with a 'candidate' label"
[ -n "$CAND" ] || { echo "!! no candidate — aborting before the PO-gate turn (issue-list stderr: $(why))"; finish; exit 1; }
