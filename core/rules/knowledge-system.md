...
Reading the map once at startup isn't enough: a long conversation shifts topics, and the page that matters for the current prompt may be one the opening scan skipped — or one whose relevance the title alone doesn't reveal. So the knowledge layer is consulted **per prompt**, from three directions:
...
- **The tool (when the situation raises the question, not the prompt).** The hook answers the user's sentence, and most of the time the question is yours rather than theirs — an instruction carries no question at all. [`/consult`](../skills/consult/SKILL.md) writes the search query itself, from what you are about to do. Reach for it when you are about to **assert how something works**, **propose or choose** between options, **edit a file whose conventions you have not verified**, or **hit an error that feels familiar** — and when an instruction names work that has a **procedure** (a release, a merge, a sweep), which carries no question but does have a checklist. Draw the query's terms from the always-loaded `wiki:index`, as terms and in English: the corpus is English, and a query written in the corpus's own vocabulary is most of what makes this worth invoking. **If none of those is true, do not consult** — a channel that fires on every turn stops being read, and this project has that measured.
