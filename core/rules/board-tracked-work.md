...
5. **On resume, check the board first — but let it inform, not dictate.** When you resume work in a board-backed project (a "continue" after a break), first check the board for items already *In Progress* and close the loop on anything left mid-flight: a session that ends mid-task (a context limit, a shutdown) leaves its item In Progress, and the next session must not lose it. This is the read complement of the write discipline above — without it, the board silently lies at the session boundary. The board tells you **what is in flight**; it does **not** by itself dictate **what is next**. A backlog may be mostly deferred, trigger-gated design work rather than shovel-ready tasks, so for *what to do next* consult the project's own resume convention (its tracker docs, recent history, active brainstorms), not the top card. Board-aware, not board-driven.
