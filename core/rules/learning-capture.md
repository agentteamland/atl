...
The watchdog measures its dry stretch **per channel**, so the signal names the channel that went dry and the drain to spawn for it. The `(learning)` variant is the one this rule owns; a `capture-watchdog (profile-fact)` signal points at `/profile-drain` and is owned by the `profile-capture` rule, exactly as the auto-drain signals split. Both can appear on the same turn — they are different subagents running different skills, and a stretch that captured neither kind genuinely needs both recoveries. (Per channel is load-bearing rather than cosmetic: a single shared predicate let one learning marker reset the counter a missed profile-fact was accumulating, so a session capturing learnings regularly could never trip the watchdog for the profile channel.)
